...
exports.NotionWebviewProvider = void 0;
const vscode = require("vscode");
class NotionWebviewProvider {
    constructor(context, notionService) {
    ...
    async openPage(pageId) {
        ...
        try {
            await vscode.window.withProgress({
                ...
                const panel = vscode.window.createWebviewPanel(NotionWebviewProvider.viewType, title, vscode.ViewColumn.One, {
                    enableScripts: true,
    ...
    renderMarkdownAsHtml(title, markdownContent) {
    ...
    getWebviewContent(title, blocks, markdownContent, webview, showMarkdown = false) {
        const nonce = this.getNonce();
        return `<!DOCTYPE html>
        <html lang="en">
        <head>
    ...
    renderRichText(richText) {
        ...
        return richText.map(textItem => {
            ...
            if (textItem.annotations) {
                ...
                if (annotations.bold)
                    text = `<span class="notion-text-bold">${text}</span>`;
                if (annotations.italic)
                    text = `<span class="notion-text-italic">${text}</span>`;
                if (annotations.strikethrough)
                    text = `<span class="notion-text-strikethrough">${text}</span>`;
                if (annotations.underline)
                    text = `<span class="notion-text-underline">${text}</span>`;
                if (annotations.code)
                    text = `<span class="notion-text-code">${text}</span>`;
            }
            if (textItem.href) {
    ...
    async saveChanges(pageId, content) {
        try {
    ...
    async toggleActiveView() {
        ...
        if (state.isMarkdownView) {
            ...
            const newPanel = vscode.window.createWebviewPanel(NotionWebviewProvider.viewType, state.title, vscode.ViewColumn.One, {
                enableScripts: true,
                retainContextWhenHidden: true,
                localResourceRoots: [this.context.extensionUri]
            });
...
exports.NotionWebviewProvider = NotionWebviewProvider;
NotionWebviewProvider.viewType = 'notion.pageView';
//# sourceMappingURL=notionWebviewProvider.js.map