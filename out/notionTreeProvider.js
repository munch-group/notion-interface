...
exports.NotionPageItem = exports.NotionTreeProvider = void 0;
const vscode = require("vscode");
const Fuse = require('fuse.js');
class NotionTreeProvider {
    ...
    updateViewMode() {
        ...
        const newMode = config.get('viewMode', 'flat');
    ...
    startBackgroundCaching() {
        ...
        vscode.window.withProgress({
            ...
            try {
                await this.notionService.cacheAllPageContent(this.pages, (loaded, total) => {
                ...
                vscode.window.showInformationMessage(`✓ Content cached for ${this.pages.length} pages. Search is now faster!`, { modal: false });
            }
            catch (error) {
    ...
    async applyFilter() {
        if (!this.searchQuery) {
            ...
            const searchablePages = await Promise.all(this.pages.map(async (page) => {
                    ...
                    if (path === 'properties') {
                        ...
                        for (const [key, value] of Object.entries(extractedProperties)) {
                            ...
                            if (Array.isArray(value) && value.length > 0) {
                                const arrayValues = value.map(v => String(v).toLowerCase());
            ...
            if (this.filteredPages.length > 0) {
                const titleMatches = results.filter((r) => r.matches?.some((m) => m.key === 'title')).length;
                const contentMatches = results.filter((r) => r.matches?.some((m) => m.key === 'content')).length;
    ...
    buildPageTree() {
        ...
        console.log('Page tree built:', Array.from(this.pageTree.entries()).map(([parent, children]) => `${parent}: ${children.length} children`));
    }
    getPageById(pageId) {
...
exports.NotionTreeProvider = NotionTreeProvider;
class NotionPageItem extends vscode.TreeItem {
    constructor(page, hasChildren, searchQuery) {
        super(page.title, hasChildren ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None);
        this.page = page;
...
//# sourceMappingURL=notionTreeProvider.js.map